    ...
    <div className="w-full h-full max-w-4xl mx-auto p-6 md:p-12">
      ...
      {step === 'error' && (
        <div className="flex flex-col items-center justify-center h-full text-center">
          <div className="w-16 h-16 bg-red-500/20 text-red-500 rounded-full flex items-center justify-center mb-6 text-2xl">!</div>
          <h2 className="text-2xl font-bold mb-4">Something went wrong</h2>
          <p className="text-secondary mb-8">{errorMsg}</p>
          <button
            onClick={() => setStep('input')}
            className="px-6 py-3 bg-bg-tertiary hover:bg-border-color text-white rounded-lg transition-colors font-medium"
          >
            Try Again
          </button>
        </div>
